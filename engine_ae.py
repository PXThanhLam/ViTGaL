...
import utils
from losses import ReductionAELoss
import numpy as np
...
def train_one_epoch_ae(model: torch.nn.Module, criterion: ReductionAELoss ,
    ...
    for batch in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.cuda.amp.autocast():
            global_logit, ae_local, original_local = model(samples,targets)
            loss,global_loss,ae_loss = criterion(global_logit, ae_local, original_local,targets)
...
@torch.no_grad()
def evaluate_ae(data_loader, model, device):
    criterion = ReductionAELoss()
    ...
    for batch in metric_logger.log_every(data_loader, 50, header):
        ...
        # compute output
        with torch.cuda.amp.autocast():
            global_logit, ae_local, original_local = model(images,target)
            

        loss,global_loss,ae_loss =criterion(global_logit, ae_local, original_local,target)
